...
function TableGood({ openFormGood, priId }) {
  const [selectedPRI, setSelectedPRI] = useState([]);
  const { store, addOption, removeOption, setStore, addStore, removeStore } = useStore();
  const [selectedAll, setSelectedAll] = useState(false);


  const handleSelect = () => {
    selectedPRI.forEach((id) => {
      removeStore(id);
    })
    openFormGood();
  };
  ...
  const handleChangeSelectedPRI = (id, item) => {
    console.log(id);
    if (selectedPRI.includes(id)) {
      priId.current = priId.current.filter((item) => item != id);
      setSelectedPRI(selectedPRI.filter((item) => item !== id));
      removeOption(id);
    } else {
      priId.current = [...priId.current, id];
      setSelectedPRI([...selectedPRI, id]);
      addOption(item);
    }
  };
      ...
      <div className="bg-white w-full max-w-6xl shadow-xl rounded-sm flex flex-col border border-gray-300 z-10 overflow-y-auto h-[80vh]">
        <div class="bg-gray-100 px-4 py-2 flex justify-between items-center border-b border-gray-300">
          ...
          <div class="flex space-x-2 text-gray-400">
            ...
            <button onClick={openFormGood} class="hover:text-gray-600">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
        ...
        <div class="p-4 space-y-3 border-b border-gray-300">
          <div class="flex flex-wrap items-center gap-4 text-sm">
            ...
            <div class="flex items-center gap-2">
              <label class="whitespace-nowrap font-medium">Nhà cung cấp</label>
              <div class="relative">
          ...
          <table class="w-full border-collapse text-sm">
            <thead class="bg-gray-100 sticky top-0 z-10 shadow-sm">
              <tr>
                ...
                <th class="border border-gray-300 p-2 text-left font-semibold text-gray-700 w-42">
                  Mã phiếu
                </th>
                ...
                <th class="border border-gray-300 p-2 text-left font-semibold text-gray-700 w-30">
                  Kho báo
                </th>
                <th class="border border-gray-300 p-2 text-left font-semibold text-gray-700 w-30">
                  NCC
                </th>
                <th class="border border-gray-300 p-2 text-left font-semibold text-gray-700 w-20">
                  Số lượng
                </th>
                <th class="border border-gray-300 p-2 text-left font-semibold text-gray-700 w-20">
                  Đơn vị
            ...
            <tbody class="divide-y divide-gray-200">
              {store.length ? store.map((item) => (
                <RowGood
                  ...
                  handleChangeSelectedPRI={handleChangeSelectedPRI}
